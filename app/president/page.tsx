"use client";

import { useEffect, useMemo, useState } from "react";
import { AdminSideNav } from "@/components/AdminSideNav";
import { StatusBadge, UrgencyBadge } from "@/components/Badges";
import { CountUp } from "@/components/CountUp";
import { OpsAiAssistant } from "@/components/OpsAiAssistant";
import { RiskGaugeSemi } from "@/components/RiskGaugeSemi";
import { getAdminAiAction, getResourceAllocationPlan } from "@/lib/aiIntel";
import { useStore } from "@/lib/store";
import { datelineCountdown, isPastDateline } from "@/lib/utils";

export default function PresidentQueuePage() {
  const { complaints } = useStore();
  const [nowMs, setNowMs] = useState(() => Date.now());

  useEffect(() => {
    const timer = setInterval(() => setNowMs(Date.now()), 1000);
    return () => clearInterval(timer);
  }, []);

  const enriched = useMemo(
    () =>
      complaints.map((item) => ({
        item,
        ai: getAdminAiAction(item),
        overdue: item.status !== "DONE" && isPastDateline(item),
      })),
    [complaints],
  );

  const activeCount = enriched.filter((row) => row.item.status !== "DONE").length;
  const overdueCount = enriched.filter((row) => row.overdue).length;
  const highRiskCount = enriched.filter((row) => row.item.status !== "DONE" && row.ai.riskScore >= 80).length;

  const topHighRisk = [...enriched]
    .filter((row) => row.item.status !== "DONE")
    .sort((a, b) => b.ai.riskScore - a.ai.riskScore)
    .slice(0, 5);

  const zonePressure = useMemo(() => {
    const zoneMap = enriched
      .filter((row) => row.item.status !== "DONE")
      .reduce<Record<string, number>>((acc, row) => {
        acc[row.item.zone] = (acc[row.item.zone] || 0) + 1;
        return acc;
      }, {});
    return Object.entries(zoneMap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3);
  }, [enriched]);

  const allocationPlan = useMemo(() => getResourceAllocationPlan(complaints), [complaints]);

  const tekananOperasi = Math.min(100, Math.round((activeCount / Math.max(1, complaints.length)) * 100 + overdueCount * 6));
  const risikoKeselamatan = Math.min(100, Math.round(highRiskCount * 18 + overdueCount * 6));
  const prestasiSla = Math.max(0, 100 - Math.round((overdueCount / Math.max(1, activeCount)) * 100));
  const intelligenceIndex = Math.max(0, Math.min(100, Math.round((100 - tekananOperasi + (100 - risikoKeselamatan) + prestasiSla) / 3)));

  const liveAlert = topHighRisk[0] ?? null;
  const currentInsight = useMemo(() => {
    const items = [
      `Indeks semasa ${intelligenceIndex}/100 dengan ${highRiskCount} kes berisiko tinggi aktif.`,
      `${allocationPlan.primaryDepartment} memerlukan intervensi segera untuk menstabilkan beban operasi.`,
      `${zonePressure[0]?.[0] ?? "Bandar"} kini menunjukkan tekanan operasi tertinggi dalam bandar.`,
      `${overdueCount} kes melepasi SLA dan memerlukan perhatian kepimpinan.`,
    ];
    return items[Math.floor(nowMs / 3000) % items.length];
  }, [allocationPlan.primaryDepartment, highRiskCount, intelligenceIndex, nowMs, overdueCount, zonePressure]);

  return (
    <div className="grid gap-4 lg:grid-cols-[240px_1fr]">
      <AdminSideNav />

      <section className="grid gap-4 lg:grid-cols-[1.05fr_1fr]">
        <div className="panel president-hero border-amber-200 bg-amber-50/80 lg:col-span-2">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-amber-800">Papan Pemuka Presiden</p>
              <h1 className="mt-1 text-2xl font-black text-slate-900">Meja Keputusan Presiden</h1>
              <p className="mt-2 max-w-3xl text-sm text-slate-700">
                Fokus di sini ialah risiko bandar, tekanan jabatan, dan cadangan keputusan strategik yang dijana oleh AI.
              </p>
            </div>
            <div className="president-status-card rounded-2xl border border-slate-200 bg-white px-4 py-3">
              <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">AI Decision Status</p>
              <p className="mt-1 text-lg font-black text-slate-900">Aktif & Memantau</p>
              <p className="mt-1 text-xs text-slate-600">Model sedang menilai risiko bandar, tekanan jabatan, dan keperluan intervensi.</p>
            </div>
          </div>

          <p className="president-live-brief mt-4 rounded-2xl border border-cyan-200 bg-slate-950 px-4 py-3 text-sm text-cyan-50">
            <span className="mr-2 inline-flex rounded-full bg-cyan-400/15 px-2 py-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-cyan-200">AI Briefing</span>
            {currentInsight}
          </p>

          <div className="mt-3 grid gap-2 md:grid-cols-4">
            <div className="rounded-xl border border-white bg-white px-3 py-2 text-sm text-slate-700">Indeks Risiko Bandar AI</div>
            <div className="rounded-xl border border-white bg-white px-3 py-2 text-sm text-slate-700">Cadangan Agihan Sumber AI</div>
            <div className="rounded-xl border border-white bg-white px-3 py-2 text-sm text-slate-700">Ringkasan Mingguan AI</div>
            <div className="rounded-xl border border-white bg-white px-3 py-2 text-sm text-slate-700">Cadangan Siaran AI</div>
          </div>
        </div>

        <div className="panel panel-ai president-command">
          <p className="text-xs font-bold uppercase tracking-wide text-cyan-200">Presiden View</p>
          <h2 className="mt-1 text-2xl font-extrabold text-white">Indeks Kecerdasan Bandar</h2>
          <p className="mt-2 text-sm text-slate-300">AI menggabungkan tekanan operasi, risiko keselamatan, dan prestasi SLA menjadi satu skor strategik.</p>

          <div className="mt-4 flex justify-center">
            <RiskGaugeSemi score={intelligenceIndex} large label="CITY INTELLIGENCE INDEX" />
          </div>

          <div className="mt-4 rounded-2xl border border-cyan-400/15 bg-white/8 px-4 py-3 text-center">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-200">Status Strategik</p>
            <p className="mt-1 text-lg font-black text-white">
              {intelligenceIndex >= 75
                ? "Bandar stabil dengan pemantauan aktif"
                : intelligenceIndex >= 50
                  ? "Bandar stabil tetapi tekanan meningkat"
                  : "Bandar memerlukan intervensi segera"}
            </p>
          </div>

          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-white/15 bg-white/10 p-3">
              <p className="text-sm font-semibold text-cyan-100">Tekanan Operasi</p>
              <p className="mt-1 text-3xl font-extrabold text-white"><CountUp target={tekananOperasi} /></p>
            </div>
            <div className="rounded-xl border border-white/15 bg-white/10 p-3">
              <p className="text-sm font-semibold text-cyan-100">Risiko Keselamatan</p>
              <p className="mt-1 text-3xl font-extrabold text-white"><CountUp target={risikoKeselamatan} /></p>
            </div>
            <div className="rounded-xl border border-white/15 bg-white/10 p-3">
              <p className="text-sm font-semibold text-cyan-100">Prestasi SLA</p>
              <p className="mt-1 text-3xl font-extrabold text-white"><CountUp target={prestasiSla} suffix="%" /></p>
            </div>
          </div>
        </div>

        <div className="grid gap-4">
          <div className="panel president-priority-card border-red-200 bg-gradient-to-r from-red-50 to-orange-50">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-red-700">Kes Utama Semasa</p>
                <h3 className="mt-1 text-xl font-extrabold text-slate-900">{liveAlert ? liveAlert.item.id : "Tiada amaran aktif"}</h3>
              </div>
              {liveAlert ? (
                <div className="rounded-2xl border border-red-200 bg-white px-4 py-3 text-center">
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Skor Risiko</p>
                  <p className="text-3xl font-black text-red-700"><CountUp target={liveAlert.ai.riskScore} /></p>
                </div>
              ) : null}
            </div>
            {liveAlert ? (
              <div className="mt-3 grid gap-2 text-sm text-slate-700">
                <p><span className="font-semibold">Zon:</span> {liveAlert.item.zone}</p>
                <p><span className="font-semibold">Kategori Rasmi:</span> {liveAlert.item.rasmiJenisAduan}</p>
                <p><span className="font-semibold">Saranan AI:</span> Utamakan intervensi kepimpinan dan percepatkan tindakan lapangan.</p>
              </div>
            ) : (
              <p className="mt-3 text-sm text-slate-700">Semua indikator berada pada tahap terkawal.</p>
            )}
          </div>

          <div className="panel border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-amber-700">Cadangan Agihan Sumber AI</p>
                <h3 className="mt-1 text-xl font-extrabold text-slate-900">{allocationPlan.primaryDepartment}</h3>
              </div>
              <div className="rounded-2xl border border-amber-200 bg-white px-4 py-3 text-center">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Skor Beban</p>
                <p className="text-3xl font-black text-amber-700"><CountUp target={allocationPlan.overloadScore} /></p>
              </div>
            </div>
            <p className="mt-3 text-sm font-semibold text-slate-700">{allocationPlan.recommendation}</p>
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <article className="rounded-xl border border-slate-200 bg-white p-3">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Kes Keutamaan</p>
                <p className="mt-1 text-3xl font-black text-red-700"><CountUp target={allocationPlan.priorityCases} /></p>
              </article>
              <article className="rounded-xl border border-slate-200 bg-white p-3">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Tangguh Tugas Rutin</p>
                <p className="mt-1 text-3xl font-black text-slate-900"><CountUp target={allocationPlan.deferredTasks} /></p>
              </article>
              <article className="rounded-xl border border-slate-200 bg-white p-3">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Tindakan Disyorkan</p>
                <p className="mt-1 text-sm font-semibold text-slate-700">Intervensi penyelia</p>
              </article>
            </div>
            <div className="mt-3 grid gap-2 text-sm">
              {allocationPlan.interventionPlan.map((item) => (
                <p key={item} className="rounded-xl border border-amber-200 bg-white px-3 py-2 text-slate-700">{item}</p>
              ))}
            </div>
          </div>
        </div>

        <div className="lg:col-span-2">
          <OpsAiAssistant scope="president" complaints={complaints} />
        </div>

        <div className="panel president-table-wrap overflow-x-auto">
          <h3 className="text-xl font-extrabold text-slate-900">5 Kes Risiko Tertinggi</h3>
          <p className="mt-1 text-sm text-slate-600">Senarai ini disusun oleh AI mengikut risiko operasi dan ancaman kepada SLA.</p>
          <table className="mt-3 min-w-full text-left text-sm">
            <thead>
              <tr className="admin-head border-b border-slate-300">
                <th className="py-2 pr-3">ID</th>
                <th className="py-2 pr-3">Zon</th>
                <th className="py-2 pr-3">Risiko</th>
                <th className="py-2 pr-3">Keutamaan</th>
                <th className="py-2 pr-3">Status</th>
                <th className="py-2 pr-3">SLA</th>
              </tr>
            </thead>
            <tbody>
              {topHighRisk.map(({ item, ai, overdue }) => (
                <tr key={item.id} className={overdue || ai.riskScore >= 80 ? "high-risk-row live-monitor-row border-b border-slate-200" : "border-b border-slate-200"}>
                  <td className="py-2 pr-3 font-bold text-blue-900">{item.id}</td>
                  <td className="py-2 pr-3">{item.zone}</td>
                  <td className="py-2 pr-3">
                    <div className="risk-score-wrap">
                      <span className="risk-score-chip">{ai.riskScore}</span>
                      <span className="risk-mini-track">
                        <span className="risk-mini-fill" style={{ width: `${ai.riskScore}%` }} />
                      </span>
                    </div>
                  </td>
                  <td className="py-2 pr-3"><UrgencyBadge urgency={item.aiUrgency} /></td>
                  <td className="py-2 pr-3"><StatusBadge status={item.status} /></td>
                  <td className={`py-2 pr-3 font-semibold ${overdue ? "text-red-700" : "text-slate-700"}`}>{datelineCountdown(item, nowMs)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>

        <div className="panel president-zone-panel">
          <h3 className="text-xl font-extrabold text-slate-900">3 Kawasan Tekanan</h3>
          <div className="mt-3 grid gap-3">
            {zonePressure.length ? (
              zonePressure.map(([zone, count], idx) => (
                <div key={zone} className="president-zone-card rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <p className="text-sm font-bold uppercase tracking-wide text-slate-500">#{idx + 1} {zone}</p>
                  <p className="text-3xl font-extrabold text-slate-900"><CountUp target={count} /></p>
                  <p className="text-sm font-semibold text-slate-600">Aduan aktif</p>
                </div>
              ))
            ) : (
              <p className="text-sm font-semibold text-emerald-700">Tiada tekanan operasi aktif.</p>
            )}
          </div>
        </div>
      </section>
    </div>
  );
}
